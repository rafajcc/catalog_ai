...
driver works. `DB_TYPE` is the **only** selector and it is **mandatory**: the app
refuses to boot without it. Pick the embedded SQLite file or point at a **managed
MySQL / MariaDB** — **without changing a single business SQL statement, function
signature, row shape or synchronous call**.

## Status

- Implemented. `DB_TYPE=sqlite` boots the embedded SQLite (`sql.js`);
  `DB_TYPE=mysql|mariadb` switches to MySQL/MariaDB (requires the `DB_*` group).
  PostgreSQL is not implemented yet and fails at boot with a clear error.
...
|---|---|---|
| `DB_TYPE` | `sqlite` \| `mysql` \| `mariadb` | **Mandatory** engine selector: sqlite (embedded) or external MySQL/MariaDB. Unset/empty ⇒ boot error. `postgres`/unknown ⇒ boot error. |
| `DB_HOST` | string | External host (socket path). Required for `DB_TYPE=mysql`/`mariadb`. |
| `DB_PORT` | number (`3306`) | External port. Optional, defaults to 3306. |
| `DB_NAME` | string | Database name. Required for external. |
| `DB_USER` | string | Login user. Required for external. |
| `DB_PASSWORD` | string | Password. Required for external. |
| `DB_SSL` | `false` \| `true` | TLS for the connection. |
| `DB_MAX_POOL` | number (`10`) | Connection-pool size of the worker. |
| `DATA_DIR` | path | Used only by sqlite: location of `catalogai.db`; default location when unset. |

Resolution rules (`resolveDbDialect`):

- `DB_TYPE=sqlite` → embedded SQLite; ignores any residual `DB_*` variable.
- `DB_TYPE=mysql|mariadb` → external MySQL/MariaDB; requires `DB_HOST`+`DB_NAME`+
  `DB_USER`+`DB_PASSWORD` (missing any of them throws a boot error listing them).
- Unsupported dialect or absent `DB_TYPE` throw a descriptive error at boot
  instead of silently running on the wrong backend.
...
- Production keeps working with `DB_TYPE=sqlite` and `DATA_DIR` pointing at a mounted volume.
- For a managed MySQL/MariaDB: set `DB_TYPE=mysql` (or `mariadb`) plus the `DB_*`
  group and the app creates/synchronizes the schema on boot. `persist()` is a
