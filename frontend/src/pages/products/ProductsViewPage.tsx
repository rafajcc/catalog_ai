...
// only the text fields the user actually changed, so the grid can highlight
// them. Image changes are computed separately by mergeImageEdits because the
// editor contents do not carry the pending AI-added images.
...
// Whether an image id was generated by the merge (an AI or local fallback id)
// rather than being a real PrestaShop image id.
function isGeneratedImageId(id: string): boolean {
  return id.startsWith('ai-') || id.startsWith('local-');
}

// Recovers the index of a generated (`ai-`/`local-`) image id within the
// pending image_urls / local_images lists. The id embeds the position the
// image had in the merged list (after the real images), which is the same
// offset used by mergeProductEdits.
function generatedImageIndex(id: string, realCount: number, aiCount: number): number | null {
  const match = /-(\d+)$/.exec(id);
  if (!match) return null;
  const position = Number(match[1]);
  if (id.startsWith('ai-')) return position - realCount;
  if (id.startsWith('local-')) return position - (realCount + aiCount);
  return null;
}

// Rebuilds the image portion of the edits when saving from the editor modal.
// The modal only knows the newly uploaded local files and the ids of images
// marked for deletion, while the AI-added images live in the pending
// `image_urls`. Without carrying them forward, a text-only save would replace
// the whole product edit and silently drop the previously added images.
function mergeImageEdits(
  textEdits: ProductEdits,
  merged: ImportedProduct,
  formImagesToDelete: string[],
  formLocalImages: ProductImageUpload[],
  previous: ProductEdits
): ProductEdits {
  const mergedImages = merged.images ?? [];
  const realCount = mergedImages.filter((img) => !isGeneratedImageId(img.id)).length;
  const aiCount = mergedImages.filter((img) => img.id.startsWith('ai-')).length;

  const previousUrls = previous.image_urls ?? [];
  const previousLocals = previous.local_images ?? [];
  const previousDeletions = previous.images_to_delete ?? [];

  const removedUrlIndices = new Set<number>();
  const removedLocalIndices = new Set<number>();
  for (const id of formImagesToDelete) {
    const index = generatedImageIndex(id, realCount, aiCount);
    if (index === null) continue;
    if (id.startsWith('ai-')) removedUrlIndices.add(index);
    else if (id.startsWith('local-')) removedLocalIndices.add(index);
  }

  const urls = previousUrls.filter((_, i) => !removedUrlIndices.has(i));
  const deletions = [...new Set([...previousDeletions, ...formImagesToDelete.filter((id) => !isGeneratedImageId(id))])];
  const locals = [...previousLocals.filter((_, i) => !removedLocalIndices.has(i)), ...formLocalImages];

  const next: ProductEdits = { ...textEdits };
  if (urls.length > 0) next.image_urls = urls;
  if (locals.length > 0) next.local_images = locals;
  if (deletions.length > 0) next.images_to_delete = deletions;
  return next;
}
...
export default function ProductsViewPage({
  ...
  function handleSave(original: ImportedProduct, fields: ProductEditForm) {
    ...
    // previously pending edits of the same product. Image edits are carried
    // forward on top of that diff so AI-added images survive a text-only save.
    const raw = (products ?? []).find((p) => p.id === original.id) ?? original;
    const next = mergeImageEdits(
      diffEdits(raw, fields),
      original,
      fields.imagesToDelete,
      fields.localImages,
      edits[original.id] ?? {}
    );
    onSaveProduct(original.id, next);
