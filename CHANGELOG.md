...
- **Per-admin user management isolation.** A comercio admin can reset the passwords of regular users of their own comercio only: modifying another admin (role toggle, password reset, deletion) is rejected with 403, and only the super admin can touch admins. Registering an already-registered comercio remains impossible.
