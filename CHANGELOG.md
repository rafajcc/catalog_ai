...
- **MySQL/MariaDB persistence layer (implemented).** The database layer designed in the earlier "Database layer design" entry is now implemented: `DB_TYPE=mysql|mariadb` or a complete `DATABASE_URL`/`DB_URL` (`mysql://user:pass@host:port/db`) boots the app against a managed MySQL/MariaDB instead of the embedded SQLite file. The domain layer keeps its exact synchronous API — the async `mysql2/promise` client lives in a `worker_thread` reached through a `MessageChannel` port + `SharedArrayBuffer`/`Atomics.wait`. SQLite stays the default with zero configuration changes. See `docs/DATABASE.md` for the translation table and the opt-in `MYSQL_TEST_URL` integration test.
